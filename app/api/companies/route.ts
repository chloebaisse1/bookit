...
export async function PATCH(request: Request) {
  try {
    ...
    if (!company) {
      return NextResponse.json(
        { messsage: "Entreprise introuvable" },
        { status: 404 }
      )
    }
    ...
    if (company.createdById !== creator.id) {
      return NextResponse.json(
        { messsage: "Vous n'etes pas le créateur de l'entreprise" },
        { status: 403 }
      )
    }
    ...
    if (action === "ADD") {
      ...
      if (employee?.companyId && employee.companyId !== company.id)
        return NextResponse.json(
          { message: `Cet employé appartient déjà a une autre entreprise` },
          { status: 400 }
        )

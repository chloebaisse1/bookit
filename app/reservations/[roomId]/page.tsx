            ...
            <div className="w-full h-fit">
              {mergedSlots.length > 0 && (
                <ul className="hidden md:flex flex-wrap gap-1 mb-4 items-center">
                  ...
                  {mergedSlots.map((mergedSlot, index) => (
                    <li key={index} className="badge badge-ghost">
                      {mergedSlot}
                    </li>
                  ))}
                </ul>
              ...
              <div className="flex">
                <div className="md:border-base-300 md:border md:rounded-xl md:p-5 h-fit md:w-1/3">
                  <Image
                    src={
                      roomData.room.imgUrl
                        ? roomData.room.imgUrl
                        : "/placeholder.jpg"
                    }
                    alt={roomData.room.id}
                    width={400}
                    height={400}
                    quality={100}
                  ...
                  <button
                    className="btn btn-outline mt-4 btn-sm btn-secondary md:hidden block"
                    onClick={() =>
                      (
                ...
                <div className="hidden md:block ml-4 w-2/3">
                  <div className="flex">
                    <input
                      type="date"
                      value={selectedDate}
                      placeholder="Choisissez une date de réservation"
                  ...
                  <ul className="grid grid-cols-2 gap-4 mt-4">
                    {availableSlots.length > 0 ? (
                      availableSlots.map((slot, index) => (
                        <button
                          className={`btn w-full btn-md ${
                            isSlotSelected(slot)
                              ? "btn-secondary"
                              : "btn-outline btn-ghost border border-base-300 text-gray-500 hover:bg-secondary hover:border-secondary"
                          } `}
                      ...
                      <p className="">Aucuns créneaux disponible.</p>
                    )}
                  </ul>
                </div>
              </div>
